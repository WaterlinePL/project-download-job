...
do
    ...
    mc cp minio/hydrological-simulations/models/$model_type/$model_name.zip /workspace/$project_name/$model_type/$model_name.zip
    ...
    if [ $model_type = "hydrus" ]; then
        mc cp minio/hydrological-simulations/projects/hydrus-shapes/$1_$model_name.npy /workspace/$project_name/hydrus_shapes/$model_name.npy
    fi
done
